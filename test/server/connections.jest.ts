...
describe('MYIOServer Connections', () => {
    it('basic launch and stop', async () => {
        const server = new MYIOServer({ port: TESTPORT, output })
        await server.launch()
        await server.stop()
    })

    it("launching 2 seperate servers should be possible", async () => {
        const server1 = new MYIOServer({ port: TESTPORT, output })
        await server1.launch()
        await server1.stop()

        const server2 = new MYIOServer({ port: TESTPORT + 1, output })
    ...
    it("Launching 2 seperate servers on same port should fail", async () => {
        ...
        const server2 = new MYIOServer({ port: TESTPORT, output })
